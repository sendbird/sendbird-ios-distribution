...
`Package.swift` also publishes `SendbirdMarkdownUI` and `SendbirdNetworkImage` for SwiftPM. From tag `1.0.0` they are **prebuilt XCFrameworks** (binary targets) attached to the GitHub release of the same tag, together with `Splash.xcframework.zip`. This lets apps built with Xcode 27 keep an iOS 14.0 deployment target; Xcode 27 cannot compile source packages below iOS 15.0. You normally do not add this package directly — `SendbirdAIAgentCore` depends on it. The XCFrameworks are built with Xcode 26 by `scripts/build_xcframeworks.sh` (see `AGENTS.md`). CocoaPods installs keep building from `Sources/`.
